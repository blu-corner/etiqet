...
public class EtiqetHandlers {
  ...
  /**
   * Gets the value from a Cdr object based on a given key. Allows use of nested elements to return
   * a value.
   *
  ...
  /**
   * Method to extract the timestamp from a message and the timestamp from cucumber and convert to
   * long millis
   *
   ...
   * @param skipOther skip any other messages found in the queue, rather than stopping and failing
   * the test.
   */
   ...
   * @param position optional if split set, compare the result at this position from the split
   * value.
  ...
  /**
   * Method to split a value if the string to split on exists in the value, otherwise return the
   * original value
   *
  ...
  /**
   * Provides the fully qualified URL object for opening a connection. Abstracted for help with unit
   * testing
   *
