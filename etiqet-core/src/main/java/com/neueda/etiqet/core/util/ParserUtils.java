...
public class ParserUtils {
  ...
  private static CdrItem getCdrItem(String key, String rest, String value, Cdr parent) {
    ...
    if (StringUtils.isNullOrEmpty(rest)) {
      ...
      Cdr cdrChild;
      if (item != null) {
        List<Cdr> children = item.getCdrs().stream().filter(x -> x.containsKey(tag[0]))
  ...
  /**
   * By default, when creating a CdrItem, the type is set to CDR_STRING and never changed in {@link
   * #getCdrItem} This means every non-String CdrItem is handled incorrectly.
   *
   ...
   *
   * @param params String containing a list of comma separated key values.
   * @param values Optional list of values to populate attributes with, will override values in
   * params.
   * @return CDR message containing the parsed parameters.
   */
  ...
  public static void fillDefaultHeaders(Message message, Cdr cdr) throws EtiqetException {
    if (message != null && message.getHeaders() != null
        && message.getHeaders().getField() != null) {
  ...
  /**
   * This method is used to check a field matches a given date/datetime format logging a warning
   * were appropriate
   *
   ...
   */
  public static Boolean matchDateTimeFormat(String error, String fieldName, String fieldType,
  ...
  private static void handleDefaultField(Field field, Cdr cdr) throws EtiqetException {
    ...
    switch (fieldType) {
      ...
      case "date":
        ...
        break;
      case "datetime":
        matchDateTimeFormat(ERROR_FORMAT, fieldName, fieldType, value.toString(),
