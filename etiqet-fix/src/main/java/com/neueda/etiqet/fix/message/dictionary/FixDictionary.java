...
public class FixDictionary extends AbstractDictionary {
  ...
  private static final Logger logger = LoggerFactory.getLogger(FixDictionary.class);
  ...
  public FixDictionary(String dictionaryPath) {
    ...
    try {
      ...
      Fix dictionary = (Fix) jaxbUnmarshaller
          .unmarshal(new File(Environment.resolveEnvVars(dictionaryPath)));
      ...
      if (dictionary.getHeader() != null
          && dictionary.getHeader().getField() != null
          && dictionary.getHeader().getField().length > 0) {
